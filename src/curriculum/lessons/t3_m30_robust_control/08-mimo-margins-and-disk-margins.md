...
::: check
A vehicle's classical analysis reports $8\,\mathrm{dB}$ gain margin and $55^\circ$ phase margin on every axis, and it goes unstable in flight. Give three explanations consistent with those numbers.
:::

::: answer
First, coupling: those are per-axis, one-at-a-time numbers, and the multi-loop disk margin may be a fraction of them, as the momentum-bias example showed. The fix is a multi-loop disk margin or a $\mu$ analysis with the actual actuator block structure. Second, the operating point: margins are computed on a linear model frozen at one flight condition, and the vehicle traverses an envelope in which mass, inertia, dynamic pressure and modal frequencies all move. The fix is to sweep the margin across the whole trajectory and across the dispersion, not to compute it once. Third, nonlinearity: actuator rate limits, thruster minimum impulse, propellant slosh and a structural mode whose frequency shifted with tank level all invalidate the linear analysis outright, and a rate-limited actuator in particular behaves as an enormous phase lag that no linear margin predicts. The fix is Monte Carlo on the nonlinear six-degree-of-freedom simulation, and a describing-function analysis of the limit cycle. All three are routine findings in flight anomaly investigations, and a serious programme runs all three antidotes rather than choosing between them.
:::
