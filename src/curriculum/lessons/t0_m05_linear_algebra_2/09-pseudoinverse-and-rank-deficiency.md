...
- **An over-parameterised model.** A scale factor and a bias fitted to data from one operating point, or a high-degree polynomial fitted to a short arc, give columns that are nearly linear combinations of one another.

The first two are *structural*: the deficiency is exact, it is there in the mathematics, and more data never removes it. The last two are *numerical*: the singular values are small but nonzero, and you must decide what counts as zero.
...
Choosing the tolerance is an engineering decision. The default in `np.linalg.pinv` and `np.linalg.matrix_rank` is a few times $\max(m, n)\,\varepsilon\,\sigma_1$ with $\varepsilon \approx 2.2\times 10^{-16}$, which discards only what floating-point round-off has already destroyed. That is right for a matrix known exactly and wrong for a measurement geometry: above, every singular value sat far above machine precision and the cut had to be made at $0.05\,\sigma_1$ on physical grounds. Set the threshold from your error budget, log the discarded singular values, and treat a rank drop as an event the system reports rather than absorbs silently.
...
Truncation is the crudest of the regularisers. Tikhonov regularisation replaces $1/\sigma_i$ by $\sigma_i/(\sigma_i^2 + \lambda^2)$, rolling off smoothly instead of switching, and a Kalman filter's prior covariance does the same job automatically: the prior supplies information in exactly the directions the measurement does not, so the filter never divides by a tiny singular value at all. The diagnosis is what matters here, and it is the same whichever cure you pick: a small $\sigma_i$ is a direction the data does not constrain.
...
::: answer
With all four wheels the roll command $\boldsymbol{\tau} = (0.05, 0, 0)^\mathsf{T}\,\mathrm{N\,m}$ costs $\lVert\mathbf{u}\rVert = 0.0433\,\mathrm{N\,m}$ spread evenly, $0.02165$ per wheel. Using only wheels 1, 2 and 3 forces $\mathbf{u} = (0.0433, -0.0433, 0)^\mathsf{T}$: two wheels carry twice the torque each and the effort rises to $0.0612\,\mathrm{N\,m}$, $41\%$ more. Minimum norm is exactly the statement that spreading the load over the redundant set is cheapest in the sum-of-squares sense, so peak wheel torque, peak current and bearing wear are all lower. The cold spare buys nothing in conditioning either: four wheels give three singular values of $1.155$, three wheels give $1.155$, $1.155$ and $0.577$.
:::
