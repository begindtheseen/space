...
The projects that come later in this module — a 6-DOF simulation, a powered-descent guidance solver, an attitude-estimation filter, an orbit-determination fit, a momentum-management study — are chosen because they are technically substantial and because this curriculum already teaches the material behind each one. Choosing the right project is only half the problem; the other half, where most self-taught engineers slip first, is documenting it so it survives the conversation that follows. Every lesson after this one either builds one of the five anchor projects to this standard or covers one part of getting there.

## Few and deep: what the interview format actually rewards

A portfolio review for a GNC role is rarely a quick tour of everything you have built. It is closer to a thesis defence compressed into fifteen or twenty minutes on a single project: you present it, and a panel spends most of the remaining time asking about the parts you did not choose to present — the alternative you rejected, the case that broke first, the number you cannot immediately produce. That format has a direct consequence for how you should spend your time: a project you can defend for an hour, from five different angles, without running out of real content, is worth more than five projects you can each describe for two minutes, because the two-minute version is exactly what gets tested past its depth in minute three.

This is why "few" and "deep" come first rather than as an afterthought. They describe the one property the interview format actually measures — how far your understanding extends past the point where the description stops. Real depth exists because building the project forced you to make decisions: this integrator and not that one, this measurement model and not a simpler one, this stopping criterion and not an arbitrary iteration count. Each decision is a question you have already, in effect, answered once. A project assembled quickly to fill a portfolio slot was never forced to make those decisions, so there is nothing underneath the description to ask about.

The practical implication is one of the harder ones to act on honestly: three projects, each demonstrating something different and each built to a standard where you could talk about nothing else for an hour, is a stronger portfolio than ten at half that standard — even though the ten-project version covers more topics and looks more prolific in a repository listing. Depth is not visible in a listing; it becomes visible exactly when someone asks the second question, which is precisely where a portfolio built for breadth runs out of road.
