...
This module's Powered Explicit Guidance lesson computed a single guidance cycle's steering law from a flat-gravity, drag-free local model — the exoatmospheric stage-2 burn's very first cycle, solved for $(A,B,t_{go}) = (6.4124,\, -0.020982,\, 354.09\ \mathrm{s})$. That local model, asked to predict its own trajectory's outcome, believed it would reach the target orbit to within 1.6 km of altitude and a few hundredths of a metre per second of velocity — an excellent answer, on its own terms. Flown open loop against the true, curved-gravity dynamics for the entire 354 s burn, with no re-solving, the same steering law instead missed by 163.8 km of altitude, 343.7 m/s of tangential speed, and 1238.7 m/s of residual radial velocity.
