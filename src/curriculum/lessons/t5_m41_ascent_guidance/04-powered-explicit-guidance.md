...
Take the very first cycle's solved steering — $(A,B,t_{go}) = (6.4124,\, -0.020982,\, 354.09\ \mathrm{s})$ — and fly it open loop: commit to that pitch history for the entire burn, never re-solving. The local flat-gravity model that produced these numbers believes it will hit the target exactly, because that is what it was solved to do — carried to full numerical precision, its own predicted terminal velocity and altitude match the target to better than $10^{-9}$ in relative terms, the residual of the equation solver and nothing more. Integrate the *true* nonlinear, curved-gravity equations of motion under that same fixed steering law for the same 354 s, and the vehicle actually arrives 163.8 km too high, 343.5 m/s short on tangential speed, and with 1238.6 m/s of residual radial velocity — nowhere near a circular orbit. The local model was not wrong about *itself*; it was wrong about how long its own assumptions stay valid. PEG never asks it to be right for 354 seconds. It asks it to be right for the next 5, discards the rest, and asks again.
:::
...
| Open-loop-for-the-whole-burn failure mode | same first-cycle steering, flown without re-solving: 163.8 km, 343.5 m/s, 1238.6 m/s off target |
