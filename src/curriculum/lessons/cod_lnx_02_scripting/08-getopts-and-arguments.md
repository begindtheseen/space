...
The other subject here is one line long and accounts for a whole class of bug: `"$@"` forwards a script's arguments unchanged and `"$*"` does not. Every wrapper script you write — around a simulator, a solver, a container run — hinges on getting that right, because a single wrong character turns `--out "entry burn 01"` into three arguments and a file called `entry`.

All output below was produced on this machine and pasted verbatim, with GNU bash 5.2.21, ShellCheck 0.9.0 and util-linux `getopt` 2.39.3 on Ubuntu 24.04.4.
