...
The invariants first. $H = \sqrt{1^2(1.0)^2 + 2^2(0.01)^2 + 3^2(0.01)^2} = 1.00065\,\mathrm{N\,m\,s}$, and it holds to $2.5\times 10^{-12}$ over the whole run. $T$ starts at $0.50025\,\mathrm{J}$, which is $H^2/(2I_1) = 0.50065$ to within the small transverse contribution, and the two bounds are $H^2/(2I_1) = 0.5007\,\mathrm{J}$ and $H^2/(2I_3) = 0.1669\,\mathrm{J}$.

Now the path. For the first 300 s almost nothing visible happens: $\omega_1$ falls only from $1.0000$ to $0.9815\,\mathrm{rad/s}$ and $T$ from $0.50025$ to $0.48798\,\mathrm{J}$. The body is still, to the eye, a minor-axis spinner. But $T$ is falling, and at $t = 731\,\mathrm{s}$ it drops below $H^2/(2I_2) = 0.25033\,\mathrm{J}$ — the separatrix value of lesson 6. At that moment the state crosses from the minor-axis family of polhodes to the major-axis family, and the transition is fast: by $t = 600\,\mathrm{s}$ the rates are $(0.617, 0.310, 0.162)$ and by $t = 800\,\mathrm{s}$ they are $(0.434, 0.085, -0.295)\,\mathrm{rad/s}$, the spin having moved bodily from axis 1 to axis 3.
