...
Errors are kinder than attitudes in one respect and harsher in another. Kinder, because an error in a working system is small, and small rotations are nearly linear: the whole apparatus of singularities and double covers relaxes when $\Phi$ is an arcminute. Harsher, because the error is a rotation between two frames, and the bookkeeping about which frame it is resolved in is a persistent source of mistakes — one that produces a controller that is stable, well behaved, and driving the wrong axes.
