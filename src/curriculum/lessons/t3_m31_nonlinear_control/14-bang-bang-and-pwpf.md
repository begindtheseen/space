...
The general switching-curve law, not only this symmetric case, was tested from an arbitrary state, $\theta_0=-0.9\,\mathrm{rad}$, $\dot\theta_0=0.25\,\mathrm{rad/s}$ — not a simple rest-to-rest problem, since the vehicle starts already moving the "wrong" way relative to a direct run at the curve. The law $u=-U_{\max}\,\mathrm{sign}(\theta-\theta_{\text{sw}})$ still drives it to $\lvert\theta\rvert,\lvert\dot\theta\rvert\lt10^{-4}$ in $3.346\,\mathrm{s}$, using exactly three torque reversals — bang, bang, bang, done.
:::
