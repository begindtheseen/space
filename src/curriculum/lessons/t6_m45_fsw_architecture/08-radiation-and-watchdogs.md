...
::: warning
A watchdog and a heartbeat are not the same guarantee. A heartbeat that only confirms a task is still scheduled — "I ran this cycle" — can be satisfied by a task that is looping through corrupted logic as easily as by one working correctly. A heartbeat that instead requires the task to report *progress* — a monotonically advancing sequence number, a specific milestone reached — narrows the gap, but even that stops short of checking correctness; it only narrows "alive" toward "alive and making some kind of forward progress," which is still not "computing the right answer."
:::
