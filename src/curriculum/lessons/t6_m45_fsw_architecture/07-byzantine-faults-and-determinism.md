...
```
Nothing here is a bug in the ordinary sense — both expressions compute a mathematically identical sum, correctly, to full floating-point precision. They simply do not produce the identical bit pattern, because floating-point addition rounds after every operation and the rounding depends on the order the additions happen in.
:::
