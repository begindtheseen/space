...
Direct transcription, as the previous lesson left it, makes both the states and the controls at every node free decision variables, tied together by defect constraints. That is one choice, not the only one. **Direct single shooting** makes only the controls free and computes the states by simulation — the same idea as the indirect shooting of two lessons ago, with one crucial difference: there is no costate anywhere in sight, no boundary value problem to root-find, only an ordinary optimisation over control values whose cost and constraints happen to require running a simulator to evaluate. It is a direct method (discretise, then optimise) that inherits shooting's oldest weakness in a new guise, and seeing exactly how is what makes multiple shooting, and the full collocation of the next lesson, worth their added bookkeeping.
