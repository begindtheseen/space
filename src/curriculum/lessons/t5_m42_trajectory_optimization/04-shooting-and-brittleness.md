...
Guessing is only half of why indirect shooting is brittle in practice. Suppose the mission also needed a path constraint this transfer never had — a maximum radial velocity, say, to keep the vehicle from diving too fast toward perigee. Indirectly, that constraint does not slot into the existing Hamiltonian: wherever it is active, the necessary conditions gain an **interior arc** with its own multiplier on the constraint, and the costate equations pick up jump conditions at the junctions where the constraint switches on and off — a fresh piece of by-hand calculus-of-variations work for every new constraint, on top of re-deriving which structure (bang-bang, singular, constrained arc) the trajectory now has. Nothing about that burden shows up in a shooting residual's convergence rate; it shows up earlier, in how much algebra has to be redone before there is a residual to converge at all. Direct transcription's answer to the same request — one more inequality constraint at every node — is deferred to this module's direct-methods lessons, but it is worth having the contrast in view now, since the costate-guessing difficulty just demonstrated is the *numerical* half of indirect brittleness and this structural burden is the *analytical* half.
...
which come in **reciprocal pairs**: $3.3766\times0.2961 = 0.99981$, $2.1443\times0.4664=1.00010$, and the complex pair sits exactly on the unit circle, its own reciprocal. This is not a coincidence — it is the defining algebraic signature of a Hamiltonian (symplectic) flow, the same structure that gives $\dot{\mathbf{x}}=\partial H/\partial\boldsymbol\lambda$, $\dot{\boldsymbol\lambda}=-\partial H/\partial\mathbf{x}$ its name. Every direction that grows under forward propagation is matched by an equally strong direction that shrinks. A guessing error with any component along the growing eigenvectors — generic, since a random guess is not aligned with the contracting subspace — gets amplified by a factor pushing $3.4$ over less than two orbits, which is the numerical fact underneath the $20\,\%$ convergence rate above.
