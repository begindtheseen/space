...
The raw data spreads over nearly $3.7\%$ of its own radius — visibly not a sphere. The fit recovers the hard-iron bias to within about $2\,\mathrm{nT}$ of its true value out of a $1563\,\mathrm{nT}$ offset, and the calibrated radius spread collapses to a tenth of a percent, essentially the injected sensor noise and nothing else: the soft-iron distortion has been undone along with the offset.
