...
A flight computer increments its mission elapsed time by 0.1 s every 100 Hz frame, and someone declares the accumulator as a 32-bit float. Two things go wrong. First, the increment itself is stored as $0.1000000014901\ldots$, high by $1.49 \times 10^{-9}\,\mathrm{s}$ per tick. Over a day, $864{,}000$ ticks, that alone gives $1.3\,\mathrm{ms}$ — annoying, not fatal.
