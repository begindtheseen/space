...
Take $h = 60\,\mathrm{s}$. The angle per step is $\theta = hn = 60 \times 1.1068 \times 10^{-3} = 0.06641\,\mathrm{rad}$. The predicted energy factor per step is $1 + \theta^2 = 1.004410$, and one revolution is $T/h = 94.6$ steps, so the linear model predicts the orbital energy scaled by $1.004410^{94.6} = 1.516$ per revolution — the oscillator's energy, which for the orbit means the magnitude of the *kinetic and potential* fluctuation, up by 52%.

The two-body problem is nonlinear, so the model is a guide, not a formula, but the run agrees in kind and size. After 95 Euler steps the vehicle is at $r = 12{,}078\,\mathrm{km}$, not $6{,}878$; the specific energy has risen from $-28.976$ to $-18.548\,\mathrm{km^2/s^2}$, a relative change $\delta\varepsilon = +0.360$, and the semi-major axis implied by that energy is $\mu/(2 \times 18.548) = 10{,}745\,\mathrm{km}$. The angular momentum has grown by 8.4%. Every one of the 95 steps pushed outward; none pulled back.
...
::: warning
A conserved energy does not mean a correct orbit. Leapfrog at $h = 60\,\mathrm{s}$ holds $\delta\varepsilon$ inside $6 \times 10^{-4}$ forever, but its numerical period differs from the true period by $O(h^2)$, so its *phase* error — where the vehicle is along the orbit — grows linearly with time, at about $15\,\mathrm{km}$ per hundred revolutions on the circular orbit. Energy is a necessary check, never a sufficient one; a comparison against an independent high-accuracy reference is still required for anything you intend to fly.
:::
...
Symplectic methods come with conditions. They need a *Hamiltonian* system — conservative forces, no drag, no thrust — and a *fixed* step: change $h$ from step to step and the shadow Hamiltonian changes with it, and the bounded-error property is lost. Higher-order symplectic methods exist (Yoshida's fourth- and sixth-order compositions of leapfrog, and the Gauss–Jackson multistep family that has propagated satellites and catalogued debris for decades) and are what production long-arc propagators use. For the two-body problem with perturbations the convention is to integrate the *perturbation* symplectically while handling the Kepler part analytically, which is a course of its own.
