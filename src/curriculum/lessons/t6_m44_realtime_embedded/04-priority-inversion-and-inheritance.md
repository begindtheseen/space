...
::: check
In the plain-mutex demonstration, $H$'s blocking time was $44\,\mathrm{ms}$ while the mutex's critical section is only $6\,\mathrm{ms}$ long. Explain concretely where the other $38\,\mathrm{ms}$ came from, in terms of which task owns each millisecond, rather than naming the phenomenon.
:::

::: answer
One millisecond came from $L$ finishing the slice of its critical section it was already partway through before $M$ arrived (from $t=2$ to $t=3$). Forty milliseconds came from $M$ running to completion — a task with no relationship to the mutex at all, only a higher priority than the task that happened to be holding it. Three milliseconds came from $L$ finishing the remainder of its critical section after $M$ finally released the processor. $1+40+3=44$; thirty-eight of those forty-four milliseconds — the $M$ portion plus none of it being mutex-related — trace to a task that never touches the lock, which is the concrete meaning of "unbounded": the number is set by $M$'s execution time, not by anything about the resource.
:::
