...
The first term vanishes by orthogonality; the second and third vanish because $\boldsymbol{\nu}_k$ is built entirely from data through step $k$, and $\mathbf{w}_k$, $\mathbf{v}_{k+1}$ are independent of all of it — the stochastic-model lesson's whiteness assumption, still doing its job many lessons after it was introduced. The same argument, applied one step further back at a time, extends to any lag: $\mathbb{E}[\boldsymbol{\nu}_j\boldsymbol{\nu}_k^{\mathsf{T}}] = \mathbf{0}$ for every $j \neq k$.
