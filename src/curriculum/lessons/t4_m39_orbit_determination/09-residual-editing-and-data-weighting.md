...
| Residual pattern rising after a moment in time | Unmodelled dynamics (often a manoeuvre), not bad data; fix the model, do not only edit harder |
...
Every editing and weighting decision in this lesson assumed the resulting fit's accuracy could be judged from Cartesian residuals and a single covariance number. The next lesson gives that covariance a frame — radial, in-track, cross-track — in which its shape, and not only its size, finally means something physical.
